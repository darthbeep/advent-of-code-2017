...
def checkForDuplicates(s):
    used = {}
    strings = s.split(" ")
    for string in strings:
        its = set(["".join(perm) for perm in itertools.permutations(string)])
        #print its
        for it in its:
            if it in used:
                return False
...
something = itertools.permutations("abc")
for it in something:
    print it

print checkForDuplicates("abcde fghij")
print checkForDuplicates("abcde xyz ecdab")
print checkForDuplicates("a ab abc abd abf abj")
print checkForDuplicates("iiii oiii ooii oooi oooo")
print checkForDuplicates("oiii ioii iioi iiio")

allwords = '''bdwdjjo avricm cjbmj ran lmfsom ivsof
...
biglist = allwords.split("\n")
ret = 0
for phrase in biglist:
    if checkForDuplicates(phrase):
        ret+=1
print ret
